...
date;rachunek;currency;broker_ticker;qty;total_cost;note
...
def load_opening_balances(conn: psycopg.Connection, path: Path = DEFAULT_PATH) -> int:
    ...
    with conn.cursor() as cur:
        for r in rows:
            ...
            title = f"Bilans otwarcia: {r['broker_ticker']} ({r['note']})"
            cur.execute(
                """
                ...
                """,
                (r["date"], r["rachunek"], r["currency"], title, -cost, ROW_TYPE, found[0],
